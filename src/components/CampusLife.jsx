...
const GRID_END   = 22;
const SLOT_MIN   = 30;
const SLOT_COUNT = ((GRID_END - GRID_START) * 60) / SLOT_MIN;
const SLOT_PX    = 24;
...
/* Supports both new schedules:[{day,start,end}] and legacy day/time fields */
function getSchedules(cls) {
  if (cls.schedules?.length) return cls.schedules;
  const days  = (cls.day  || '').split(',').map(d => d.trim()).filter(Boolean);
...
function SlotRow({ slot, onChange, onRemove, showRemove }) {
  return (
    <div className="flex items-center gap-1.5 bg-gray-50 rounded-xl px-2.5 py-2">
      ...
      <input
        ...
        placeholder="10:00"
        className="w-14 text-xs border border-gray-200 rounded-lg px-1.5 py-1.5 focus:outline-none focus:ring-1 focus:ring-indigo-100 font-mono text-center bg-white"
      />
      ...
      {showRemove && (
        <button
          type="button"
          onClick={onRemove}
          className="w-5 h-5 flex items-center justify-center text-gray-300 hover:text-red-400 transition-colors text-base leading-none ml-auto"
        >×</button>
      )}
    </div>
  );
...
function AddSlotButton({ onClick }) {
  return (
    <button
      onClick={onClick}
      className="mt-2 w-full flex items-center justify-center gap-1 py-1.5 text-xs text-indigo-500 hover:text-indigo-700 hover:bg-indigo-50 rounded-xl transition-colors border border-dashed border-indigo-200"
    >
...
/* ── Quick-add modal (drag) ── */
function QuickAddModal({ day, startSlot, endSlot, onSave, onClose }) {
  const [subject, setSubject] = useState('');
  const [room,    setRoom]    = useState('');
  const [memo,    setMemo]    = useState('');
  const [color,   setColor]   = useState(PALETTE[0]);
  const [slots,   setSlots]   = useState([{ day, start: slotToTime(startSlot), end: slotToTime(endSlot) }]);

  const updateSlot = (i, field, val) =>
    setSlots(prev => prev.map((s, j) => j === i ? { ...s, [field]: val } : s));
  const addSlot    = () => setSlots(prev => [...prev, { day: '', start: '', end: '' }]);
  const removeSlot = (i) => setSlots(prev => prev.filter((_, j) => j !== i));

  const validSlots = slots.filter(s => s.day && s.start && s.end);
  const canSave    = subject.trim() && validSlots.length > 0;

  const handleSave = () => {
    if (!canSave) return;
    onSave({ id: String(Date.now()), subject: subject.trim(), schedules: validSlots, room: room.trim(), memo: memo.trim(), color });
  };
  ...
  return (
    ...
    >
      ...
      <div className="bg-white md:rounded-2xl shadow-2xl w-full md:w-[420px] overflow-hidden rounded-b-none max-h-[90vh] flex flex-col">
        <div className="px-5 pt-5 pb-4 border-b border-gray-50 flex items-center justify-between flex-shrink-0">
          <p className="text-[9px] font-bold uppercase tracking-widest text-gray-400">강의 추가</p>
        ...
        <div className="px-5 py-4 space-y-3 overflow-y-auto flex-1">
          ...
          <div>
            <label className="block text-[9px] font-bold uppercase tracking-widest text-gray-400 mb-2">요일 / 시간 *</label>
...
function EditClassModal({ cls, onSave, onClose, onDelete }) {
  ...
  const [slots,   setSlots]   = useState(() => {
    const s = getSchedules(cls);
    return s.length > 0 ? s.map(x => ({ ...x })) : [{ day: '', start: '', end: '' }];
  });
  ...
  const handleSave = () => {
    if (!canSave) return;
    onSave({ ...cls, subject: subject.trim(), schedules: validSlots, room: room.trim(), memo: memo.trim(), color });
  };
...
function DragTimetableGrid({ timetable, onAddClass, onDeleteClass }) {
  ...
  timetable.forEach(cls => {
    getSchedules(cls).forEach(schedule => {
      if (WEEKDAYS.includes(schedule.day)) {
        classesByDay[schedule.day].push({ cls, schedule });
      }
    });
  });
  ...
  const isOccupied = (day, slot) =>
    classesByDay[day]?.some(({ schedule }) => {
      const s = timeToSlot(schedule.start);
      const e = timeToSlot(schedule.end);
            ...
            <div className="w-14 flex-shrink-0 border-r border-gray-100 relative" style={{ height: totalH + 34 }}>
              ...
              <div className="relative" style={{ height: totalH }}>
                {Array.from({ length: SLOT_COUNT }, (_, i) => (
                  i % 2 === 0 ? (
                    <div key={i} className="absolute right-2 flex items-center" style={{ top: i * SLOT_PX, height: SLOT_PX * 2 }}>
                ...
                <div className="relative" style={{ height: totalH }}>
                  {Array.from({ length: SLOT_COUNT }, (_, i) => (
                    <div key={i} className="absolute left-0 right-0 pointer-events-none" style={{
                      top: i * SLOT_PX, height: SLOT_PX,
                      borderTop: i % 2 === 0 ? '1px solid rgba(0,0,0,0.05)' : '1px solid rgba(0,0,0,0.02)',
                    }} />
                  ))}
                  ...
                  {Array.from({ length: SLOT_COUNT }, (_, slot) => {
                    ...
                    return (
                      <div
                        key={slot}
                        className={`absolute left-0 right-0 transition-colors duration-75 ${highlighted ? 'bg-indigo-100' : 'hover:bg-indigo-50/70'}`}
                        style={{ top: slot * SLOT_PX, height: SLOT_PX, cursor: 'crosshair', zIndex: 1 }}
                  ...
                  {dragState && dragState.day === day && (() => {
                    ...
                    const hi = Math.max(dragState.startSlot, dragState.currentSlot);
                    return (
                      <div className="absolute left-1 right-1 rounded-lg pointer-events-none z-20" style={{
                        top: lo * SLOT_PX + 1, height: (hi - lo + 1) * SLOT_PX - 2,
                  ...
                  {classesByDay[day].map(({ cls, schedule }) => {
                    const startSlot = timeToSlot(schedule.start);
                    const endSlot   = timeToSlot(schedule.end);
                    if (startSlot === null || endSlot === null || endSlot <= startSlot) return null;
                    const top    = Math.max(0, startSlot) * SLOT_PX;
                    const height = (endSlot - startSlot) * SLOT_PX;
                    if (height <= 0) return null;
                    return (
                      <div
                        key={`${cls.id}-${schedule.day}-${schedule.start}`}
                      ...
                      >
                        <div className="px-1.5 pt-1 min-h-0">
                          <p className="text-[10px] font-bold truncate leading-tight" style={{ color: cls.color }}>{cls.subject}</p>
...
/* ── Class card for AI parse review (keeps legacy day/time fields) ── */
...
export default function CampusLife({ apiKey, timetable: propTimetable, habits: propHabits, onTimetableChange, onHabitsChange }) {
  ...
  const [newCls, setNewCls] = useState({ subject: '', slots: [{ day: '', start: '', end: '' }], room: '', color: PALETTE[0] });
  ...
  const handleRegister = () => {
    ...
    const withIds = parsedClasses.map((c, i) => {
      const days  = (c.day  || '').split(',').map(d => d.trim()).filter(Boolean);
      const parts = (c.time || '').split('-');
      const start = parts[0]?.trim() || '';
      const end   = parts[1]?.trim() || '';
      return { ...c, id: String(Date.now() + i), schedules: days.map(day => ({ day, start, end })) };
    });
    setTimetable(withIds);
    const ttHabits = withIds.map(c => ({
      id: 'tt_' + c.id,
      name: `${c.subject} 출석하기`,
      days: [...new Set(c.schedules.map(s => s.day))],
      color: c.color || PALETTE[0],
      fromTimetable: true,
      timetableSubject: c.subject,
    }));
    const currentHabits = storage.get(KEYS.HABITS, []);
    setHabits([...ttHabits, ...currentHabits.filter(h => !h.fromTimetable)]);
    setShowUpload(false);
    setUploadImg(null);
    setParsedClasses(null);
  };
  ...
  const addManualClass = () => {
    const validSlots = newCls.slots.filter(s => s.day && s.start && s.end);
    if (!newCls.subject.trim() || validSlots.length === 0) return;
    const cls = { id: String(Date.now()), subject: newCls.subject.trim(), schedules: validSlots, room: newCls.room.trim(), color: newCls.color };
    setTimetable([...timetable, cls]);
    setNewCls({ subject: '', slots: [{ day: '', start: '', end: '' }], room: '', color: PALETTE[0] });
    setShowAddForm(false);
  };

  const updateNewClsSlot = (i, field, val) =>
    setNewCls(p => ({ ...p, slots: p.slots.map((s, j) => j === i ? { ...s, [field]: val } : s) }));
  const addNewClsSlot    = () =>
    setNewCls(p => ({ ...p, slots: [...p.slots, { day: '', start: '', end: '' }] }));
  const removeNewClsSlot = (i) =>
    setNewCls(p => ({ ...p, slots: p.slots.filter((_, j) => j !== i) }));
  ...
  const clearTimetable = () => {
    ...
    setTimetable([]);
    setHabits(storage.get(KEYS.HABITS, []).filter(h => !h.fromTimetable));
  };
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden" style={{ background: '#f8f8f6' }}>
      ...
      <div className="bg-white border-b border-gray-100 px-6 py-3 flex items-center gap-3 flex-shrink-0">
        ...
        {timetable.length > 0 && (
          <button onClick={clearTimetable} className="h-7 px-3 text-xs text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors border border-gray-200">초기화</button>
        )}
        <button
          ...
          {showAddForm && (
            <div className="bg-white rounded-2xl p-5" style={{ border: '0.5px solid rgba(0,0,0,0.06)' }}>
              ...
              <div className="space-y-3">
                ...
                <div>
                  <label className="block text-[10px] text-gray-400 font-semibold mb-2 uppercase tracking-wider">요일 / 시간</label>
                  <div className="space-y-1.5">
                    {newCls.slots.map((slot, i) => (
                      <SlotRow
                        key={i}
                        slot={slot}
                        onChange={(field, val) => updateNewClsSlot(i, field, val)}
                        onRemove={() => removeNewClsSlot(i)}
                        showRemove={newCls.slots.length > 1}
                      />
                    ))}
                  </div>
                  <AddSlotButton onClick={addNewClsSlot} />
                </div>
              </div>
              <div className="flex items-center gap-3 mt-4">
                ...
                <button
                  ...
                  disabled={!newCls.subject.trim() || newCls.slots.filter(s => s.day && s.start && s.end).length === 0}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 mt-0.5">
                          {getSchedules(cls).map((s, i) => (
                            <span key={i} className="text-[11px] text-gray-500">{s.day} {s.start}~{s.end}</span>
                          ))}
